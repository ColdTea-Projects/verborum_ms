...
Record IDs (`dictionaryId`, `wordId`) are **client-generated UUID strings** — this is what makes
offline creation possible and is non-negotiable for every client. Two exceptions, both system-owned
rows the clients never address by id: `vaultEntryId` and `tagId` are generated by the server.

After auth (§6), the *owner* of a record is always the JWT subject (BE task P3-05). Precisely, as of
2026-07-23:

- `userId` is **still required** on `POST`/`PUT /dictionaries/` (`@NotBlank` — omitting it is a
  **400**, not a silent default).
- It must equal your JWT `sub`. A different value is **403**, not silently ignored: a client sending
  the wrong owner has a bug, and quietly rewriting it would let that bug ship looking healthy.
- What is stored is always the token subject regardless.

> An earlier version of this section said a client-supplied `userId` "is ignored by the backend".
> That was never what shipped — corrected after the 2026-07-23 review. Keep sending your own `sub`.
